...
class UISprite(UIBase):
    ...
    def list_of_sprites(self, sprite_rect, image_count, color_key: (int, int, int)):
    ...
    def update(self, display_canvas: Surface):
        if self.current_sprite_index == self.sprite_count + 1:
            self.current_sprite_index = 0
        if time.time() - self.when_change >= self.sprite_frame_time:
            self.when_change += self.sprite_frame_time
